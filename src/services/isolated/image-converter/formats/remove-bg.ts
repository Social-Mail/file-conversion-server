    ...
    try {
        ...
        if (!existsSync(modelPath)) {
            ...
            if (!existsSync(modelPath)) {
                ...
                await using t = tempDiskCache.createTempFile(modelName)
                const rs = await fetch("https://github.com/danielgatis/rembg/releases/download/v0.0.0/" + modelName);
                await t.writeAll(Readable.fromWeb(rs.body as any));
                await cp(t.path, modelPath);
            }
        }
